...
class NeuralNetworkStochastic(object):
    def __init__(self, hidden_layer_sizes, lambda_val, iterations, batch_size):
        # Storage Arrays
        self.activity = []
        self.activation = []
        self.weights = []
    ...
    def initialize_weights(self, inputs, outputs):
        ...
        self.weights.append(
            np.random.rand(self.hidden_layer_sizes[len(self.hidden_layer_sizes) - 1] + 1, outputs.shape[1]))
    ...
    def calculate_partials(self, outputs):
        ...
        for i in reversed(range(1, self.layers - 1)):
            self.deltas[i] = np.dot(self.deltas[i + 1], np.transpose(self.weights[i])) * self.sigmoid_prime(
                self.activation[i - 1])
    ...
    def calculate_partial_derivatives_regularized(self, inputs):
        ...
        for i in range(0, len(self.partial_derivatives)):
            ...
            self.partial_derivatives_reg.append((1 / inputs.shape[0]) * self.partial_derivatives[i][1, :])
            # compute derivative for the rest of the nodes
            reg_temp = (1 / inputs.shape[0]) * (
                self.partial_derivatives[i][1:, :] + (self.lambda_val * self.weights[i][1:, :]))
    ...
    def error_function(self, inputs, outputs):
        example_cost = np.empty([1, inputs.shape[0]])
        for i in range(0, inputs.shape[0]):
            example_cost[0, i] = np.sum((outputs[i, :] * np.log(self.activation[self.layers - 1][i, :])) + (
                (1 - outputs[i, :]) * np.log(1 - self.activation[self.layers - 1][i, :])))
    ...
    def gradient_checker(self, outputs, inputs):
        ...
        for i in range(len(self.weights)):
            ...
            sub_avg = np.sum(np.abs(partial_derv_weight_guess[i] - self.partial_derivatives_reg[i])) / \
                      self.partial_derivatives_reg[i].size
    ...
    def train(self, inputs, outputs, learning_rate, gradient_check_on):
        ...
        for i in range(0, self.iterations):
            ...
            # batch training
            input_batch = np.empty((self.batch_size, inputs.shape[1]))
            output_batch = np.empty((self.batch_size, outputs.shape[1]))
            for j in range(self.batch_size):
                index = random.randint(0, inputs.shape[0] - 1)
                input_batch[j, :] = inputs[index, :]
                output_batch[j, :] = outputs[index, :]

            self.forward_prop(input_batch)
            self.back_propagation(input_batch, output_batch, learning_rate)
        ...
        self.forward_prop(inputs)
        self.activation.insert(0, inputs)
        print(self.error_function(inputs, outputs))
