...
class NeuralNetworkStochasticBiasValue(object):
    ...
    def calculate_partial_derivatives_regularized(self, inputs):
        ...
        for i in range(0, len(self.partial_derivatives)):
            ...
            reg_temp = (1 / self.batch_size) * (self.partial_derivatives[i] + (self.lambda_val * self.weights[i]))
    ...
    def adjust_weights_and_bias(self, learning_rate):
        for i in range(len(self.weights)):
            ...
            self.bias[i] = self.bias[i] - (learning_rate * (self.delta_bias[i + 1] / self.batch_size))
